...
public class InvoiceService {
        public ByteArrayInputStream generatePdfFromId(Contractor seller, Invoice invoice) {
                ...
                pdfInvoice.setBuyerData(str(invoice.getContractor().getName()),
                                str(invoice.getContractor().getFirstAddressLine()),
                                str(invoice.getContractor().getSecondAddressLine()),
                                str(invoice.getContractor().getNip()));
                pdfInvoice.setSellerData(str(seller.getName()), str(seller.getFirstAddressLine()),
                                str(seller.getSecondAddressLine()), str(seller.getNip()));
                pdfInvoice.setInfoData(str(invoice.getIssuePlace()), str(invoice.getIssueDate()),
                                str(invoice.getSellDate()));
                pdfInvoice.setPaymentDetails(str(invoice.getPaymentType()), str(invoice.getPaymentDate()),
